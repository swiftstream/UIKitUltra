...
final class MacOSWindowDeclarativeTests: XCTestCase {
    func testToolbarSetterPreservesNativeNilSemantics() {
        let wrapper = Window()
        let toolbar = NSToolbar(identifier: "UltraTestToolbar")
    ...
    func testWindowTabConfigurationStateBindingsRemainLive() {
        let wrapper = Window()
        let autosaveName = State<NSWindow.FrameAutosaveName>(wrappedValue: "Ultra.WindowTabs")
        let tabbingMode = State<NSWindow.TabbingMode>(wrappedValue: .preferred)
        let tabbingIdentifier = State<NSWindow.TabbingIdentifier>(wrappedValue: "Ultra.WindowTabs")
        ...
        XCTAssertEqual(wrapper.window.tabbingIdentifier, "Ultra.WindowTabs")
        XCTAssertEqual(wrapper.window.appearance?.name, .darkAqua)

        tabbingMode.wrappedValue = .disallowed
        tabbingIdentifier.wrappedValue = "Ultra.WindowTabs.Updated"
        appearance.wrappedValue = NSAppearance(named: .aqua)

        XCTAssertEqual(wrapper.window.tabbingMode, .disallowed)
        XCTAssertEqual(wrapper.window.tabbingIdentifier, "Ultra.WindowTabs.Updated")
